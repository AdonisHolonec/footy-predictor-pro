...
export function clampLambda(x) { return clamp(x, 0.2, 3.5); }
export function factorial(n) { if (n === 0 || n === 1) return 1; let r = 1; for (let i = 2; i <= n; i++) r *= i; return r; }
export function poissonP(k, lambda) { return (Math.pow(lambda, k) * Math.exp(-lambda)) / factorial(k); }
...
function samplePoisson(lambda) {
  ...
  let p = 1; let k = 0;
  do { k += 1; p *= Math.random(); } while (p > L && k < 20);
...
function monteCarloProbs(lambdaHome, lambdaAway, lambdaShared = 0.12, samples = 2200) {
  ...
  for (let s = 0; s < samples; s++) {
    ...
    const t = h + a;
    if (h > a) p1 += 1; else if (h === a) pX += 1; else p2 += 1;
    if (h > 0 && a > 0) pGG += 1;
    if (t > 2.5) pO25 += 1;
    if (t <= 3.5) pU35 += 1;
    if (t > 1.5) pO15 += 1;
  }
  return { p1: (p1 / samples) * 100, pX: (pX / samples) * 100, p2: (p2 / samples) * 100, pGG: (pGG / samples) * 100, pO25: (pO25 / samples) * 100, pU35: (pU35 / samples) * 100, pO15: (pO15 / samples) * 100 };
}
...
export function computeMatchProbs(lambdaHome, lambdaAway, seed = 0, options = {}) {
  ...
  let p1 = 0, pX = 0, p2 = 0, pO25 = 0, pU25 = 0, pGG = 0, pNGG = 0, pU35 = 0, pO35 = 0, pO15 = 0;
  let maxProb1 = -1, bestScore1 = "1-0", maxProbX = -1, bestScoreX = "0-0", maxProb2 = -1, bestScore2 = "0-1";
  for (let i = 0; i <= 7; i++) {
    for (let j = 0; j <= 7; j++) {
      const prob = bivariatePoissonP(i, j, lambdaHome, lambdaAway, correlation);
      if (i > j) { p1 += prob; if (prob > maxProb1) { maxProb1 = prob; bestScore1 = `${i}-${j}`; } }
      else if (i === j) { pX += prob; if (prob > maxProbX) { maxProbX = prob; bestScoreX = `${i}-${j}`; } }
      else { p2 += prob; if (prob > maxProb2) { maxProb2 = prob; bestScore2 = `${i}-${j}`; } }
      if (i + j > 2.5) pO25 += prob; else pU25 += prob;
      if (i + j > 3.5) pO35 += prob; else pU35 += prob;
      if (i + j > 1.5) pO15 += prob;
      if (i > 0 && j > 0) pGG += prob; else pNGG += prob;
    }
  }
  ...
  const p1Pct = blend(p1 * 100, mc.p1), pXPct = blend(pX * 100, mc.pX), p2Pct = blend(p2 * 100, mc.p2), pGGPct = blend(pGG * 100, mc.pGG), pO25Pct = blend(pO25 * 100, mc.pO25), pU35Pct = blend(pU35 * 100, mc.pU35), pO15Pct = blend(pO15 * 100, mc.pO15);
  return {
    probs: { p1: clamp(p1Pct, 0, 100), pX: clamp(pXPct, 0, 100), p2: clamp(p2Pct, 0, 100), pGG: clamp(pGGPct, 0, 100), pO25: clamp(pO25Pct, 0, 100), pU35: clamp(pU35Pct, 0, 100), pO15: clamp(pO15Pct, 0, 100) },
    bestScore: finalBestScore,
    pU35: clamp(pU35Pct, 0, 100),
    modelMeta: { method: "bivariate-poisson+monte-carlo", correlation, samples }
  };
}
...
export function extractFormMultiplier(formString) {
  ...
  let pts = 0;
  for (const char of recent) { if (char === "W") pts += 3; else if (char === "D") pts += 1; }
...
export function advancedLambdas(homeStats, awayStats, homeFormMulti, awayFormMulti) {
  const rawLambdaHome = (homeStats.gfHome + awayStats.gaAway) / 2;
  const rawLambdaAway = (awayStats.gfAway + homeStats.gaHome) / 2;
  return {
    lambdaHome: clampLambda(rawLambdaHome * homeFormMulti * 1.05),
    lambdaAway: clampLambda(rawLambdaAway * awayFormMulti * 0.95)
  };
}
