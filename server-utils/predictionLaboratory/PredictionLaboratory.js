/**
 * Prediction Laboratory — per-match diagnostic scores.
 * Pure derivation from prediction payload fields (no hardcoded league rates).
 * Safe to recompute on the client whenever the match row updates.
 */
...
function strengthToScore(atk, def, leagueAvg) {
...
function standingsSideScore(snap) {
  if (!snap) return null;
  ...
  let score = 50;
  if (rank != null && rank > 0) {
    // Lower rank → higher score (top of table ≈ 90+)
    score = clamp(100 - (rank - 1) * 4.2, 18, 96);
  }
  if (pts != null && played != null && played > 0) {
    const ppg = pts / played;
    const ppgScore = clamp(ppg * 28, 10, 98);
    score = rank != null ? (score * 0.55 + ppgScore * 0.45) : ppgScore;
  }
  return round1(score);
}

function poissonScore(row) {
...
function expectedGoalsScore(row) {
  ...
  const align =
    100 -
    clamp(
      (Math.abs(lh - hxg) + Math.abs(la - axg)) * 28,
      0,
      55
    );
...
function oddsScore(row) {
  ...
  const bookScore = clamp(books * 12, 20, 90);
  const marginScore = clamp(100 - overround * 4, 25, 95);
  return round1(bookScore * 0.45 + marginScore * 0.55);
}
...
function expectedValueScore(row) {
  const ev =
    num(row?.valueEngine?.expectedValue, null) ??
    num(row?.valueBet?.ev, null);
...
function bookmakerDifference(row) {
  ...
  // Radar: larger |model − book| edge → higher (capped)
  const score =
    absDiff == null
      ? 40
      : round1(clamp(35 + absDiff * 2.8, 8, 98));
...
function evolutionScore(stages) {
  if (!stages || stages.length < 2) return 55;
  let total = 0;
  let n = 0;
  ...
  const avgMove = n ? total / n : 0;
  // Stable pipeline → high score; big swings → lower
  return round1(clamp(100 - avgMove * 1.8, 15, 98));
}

function buildEvolution(row) {
  const stages = [];
  const push = (label, triple) => {
    if (!triple) return;
  ...
  // Market implied as terminal reference when odds exist
  ...
  // Dedupe identical consecutive stages
  const deduped = [];
...
function buildComparison(row) {
  ...
  const leagueAvg = num(row?.modelMeta?.leagueParams?.leagueAvg, 1.35) ?? 1.35;
  ...
  const rows = [
    ...
    },
    {
      ...
      homeDisplay:
        sm.atkH != null ? Number(sm.atkH).toFixed(2) : hStand != null ? `${Math.round(hStand)}` : "—",
      awayDisplay:
        sm.atkA != null ? Number(sm.atkA).toFixed(2) : aStand != null ? `${Math.round(aStand)}` : "—",
      edge:
        sm.atkH != null && sm.atkA != null
          ? round2(Number(sm.atkH) - Number(sm.atkA))
          : null
    },
    {
      ...
      edge:
        sm.defH != null && sm.defA != null
          ? round2(Number(sm.defA) - Number(sm.defH)) // lower GA better → flip
          : null
    },
    {
    ...
    },
    {
      ...
      homeDisplay:
        pctFromProb(row?.probs?.p1) != null ? `${pctFromProb(row?.probs?.p1).toFixed(1)}%` : "—",
      awayDisplay:
        pctFromProb(row?.probs?.p2) != null ? `${pctFromProb(row?.probs?.p2).toFixed(1)}%` : "—",
      edge:
...
/**
 * Build full Prediction Laboratory payload for a match prediction row.
 */
export function buildPredictionLaboratory(row) {
  ...
  const metrics = [
    ...
    { key: "confidence", label: "Confidence", value: scores.confidence, unit: "score" },
    {
      ...
      value: num(row?.valueEngine?.expectedValue ?? row?.valueBet?.ev, scores.expectedValue),
      display: (() => {
        const ev = num(row?.valueEngine?.expectedValue ?? row?.valueBet?.ev, null);
        return ev == null ? `${scores.expectedValue}` : `${ev >= 0 ? "+" : ""}${round1(ev)}%`;
      })(),
      unit: "ev"
    },
    {
...
export default buildPredictionLaboratory;
