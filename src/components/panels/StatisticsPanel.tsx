...
function KpiTile({
  ...
  return (
    <div className="lab-stat">
      ...
      <div className={`lab-stat-value text-base sm:text-xl ${toneClass}`}>{loading ? "…" : value}</div>
    </div>
  );
}
  ...
  return (
    <div className={`mt-3 grid ${maxWidthClass} grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-2.5`}>
      <KpiTile
        ...
        loading={kpiLoading}
        tone={(kpi?.roi || 0) >= 0 ? "good" : "bad"}
      />
      <KpiTile label="KPI Hit Rate" value={`${(kpi?.hitRate || 0).toFixed(2)}%`} loading={kpiLoading} />
      <KpiTile
        label="KPI Drawdown"
        ...
        tone="amber"
      />
      <KpiTile label="KPI Settled" value={`${kpi?.settled || 0}`} loading={kpiLoading} />
    </div>
  );
}
  ...
  return (
    <div
      className={`${props.alertsCollapsible ? "mt-3" : "mt-2 max-w-[760px]"} rounded-card border px-3 py-2.5 ${
        alertsSeverity === "high"
          ? "border-signal-rose/35 bg-signal-rose/10"
          : alertsSeverity === "medium"
            ? "border-signal-amber/40 bg-signal-amber/10"
            : "border-signal-sage/30 bg-signal-mintSoft/40"
      }`}
    >
      <div className="lab-section-eyebrow text-signal-inkMuted">Auto alerting</div>
      <div className="mt-1 text-xs font-medium text-signal-ink">
      ...
      <div className="mt-2.5 grid grid-cols-1 gap-2 sm:grid-cols-3">
        <label className="flex items-center gap-2 text-[10px] font-semibold text-signal-inkMuted">
          ...
          <input
            ...
            className="w-full rounded-lg border border-white/10 bg-signal-fog px-2 py-1.5 font-mono text-[11px] text-signal-ink"
          />
        </label>
        <label className="flex items-center gap-2 text-[10px] font-semibold text-signal-inkMuted">
          Drift
      ...
      <div className="mt-2.5 flex flex-wrap gap-2">
        <button
          ...
          className="rounded-lg bg-signal-petrol px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wide text-signal-void hover:bg-signal-petrolMuted disabled:cursor-not-allowed disabled:opacity-50"
        >
          Apply thresholds
        </button>
        <button
          onClick={() => void resetAlertThresholds()}
          className="rounded-lg border border-white/10 bg-signal-fog px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wide text-signal-ink hover:bg-signal-panel"
        >
          Reset defaults
        </button>
        {thresholdsSaved !== "idle" && (
          <span className="rounded-lg border border-signal-sage/30 bg-signal-mintSoft/50 px-2 py-1.5 text-[10px] font-semibold uppercase tracking-wide text-signal-sage">
      ...
      <>
        <div className="mt-4 grid max-w-full grid-cols-2 gap-2 sm:grid-cols-4 sm:gap-2.5">
          <KpiTile
            ...
            tone={(props.kpi?.roi || 0) >= 0 ? "good" : "bad"}
          />
          <KpiTile label="KPI Hit Rate" value={`${(props.kpi?.hitRate || 0).toFixed(2)}%`} loading={props.kpiLoading} />
          <KpiTile
            label="KPI Drawdown"
            ...
            tone="amber"
          />
          <KpiTile label="KPI Settled" value={`${props.kpi?.settled || 0}`} loading={props.kpiLoading} />
        </div>
        <details className="lab-card mt-3 p-3">
          <summary className="cursor-pointer font-sans text-[11px] font-semibold uppercase tracking-[0.12em] text-signal-inkMuted">
