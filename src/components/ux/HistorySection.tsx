  ...
  return (
    <section className="space-y-6">
      <header>
        ...
        <h1 className="mt-1 font-display text-[length:var(--fp-hero)] font-semibold">History</h1>
        <p className="mt-2 text-[length:var(--fp-body)] text-[var(--fp-text-muted)]">
          Predicții settled și pending din contul tău.
        </p>
      </header>
      ...
      {!rows.length ? (
        <Card>
          <p className="text-sm text-[var(--fp-text-muted)]">
            Încă nu există predicții în istoric. Rulează Predict, apoi revino după sync.
          </p>
        </Card>
      ) : (
              ...
              <Card padding="sm" className="flex flex-wrap items-center justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-0.5 text-sm text-[var(--fp-text-muted)]">
                    Top Pick: <span className="text-[var(--fp-text)]">{row.recommended?.pick || "—"}</span>
                    {row.score?.home != null && row.score?.away != null
                      ? ` · Scor ${row.score.home}-${row.score.away}`
                      : ""}
                  </p>
                </div>
