    ...
    <>
      <div className="grid grid-cols-2 items-center gap-2 sm:flex sm:flex-wrap lg:flex-nowrap lg:gap-3">
        <input
          ...
          className="col-span-2 w-full rounded-xl border glass-input px-4 py-2.5 text-sm text-[var(--fp-text)] outline-none focus:ring-2 focus:ring-[var(--fp-success)]/35 sm:col-span-1"
        />
        <button
          type="button"
          onClick={() => {
          ...
          className="touch-manipulation rounded-xl border border-white/10 bg-[var(--fp-bg-card)]/60 px-4 py-2.5 text-sm font-semibold text-[var(--fp-text)] transition-all hover:bg-[var(--fp-bg-card)] hover:text-[var(--fp-accent)] active:translate-y-px"
        >
          + Zi
        </button>
        {actions}
        ...
        {normalizeSelectedDates(selectedDates.length ? selectedDates : [date]).map((d) => (
          <button
            ...
            className={`rounded-full border px-3 py-1.5 text-[10px] font-semibold ${
              d === date ? "border-[var(--fp-accent)]/40 bg-[var(--fp-accent)]/15 text-[var(--fp-accent)]" : "border-white/10 bg-[var(--fp-bg-card)]/40 text-[var(--fp-text-muted)]"
            }`}
            title="Elimină ziua"
          >
