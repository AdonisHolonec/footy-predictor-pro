 ...
 *
 * HEADER → DECISION → WHY → LIVE (in play) → MARKETS → SPECIAL BET → CONTEXT →
 * ADVANCED. Header, Decision and the Why summary are visible; everything else
 * is a disclosure; Advanced exists only with the Account setting. Each fact
...
describe("UX-C · order and defaults", () => {
  it("[2] renders the layers in the approved order, Decision immediately after the header", () => {
    renderDetail();
    const order = layers();
    expect(order.slice(0, 3)).toEqual(["header", "decision", "why"]);
    expect(order.indexOf("markets")).toBeGreaterThan(order.indexOf("why"));
  ...
  it("[6][7] Why: the summary sentence is visible; factors and the deep explanation are collapsed", () => {
    renderDetail();
    const w = layer("why")!;
...
describe("UX-C · Live layer", () => {
  ...
  it("[9][10] is present in play, right after Why, compact by default, details behind one disclosure", () => {
    renderDetail({
      ...LIVE,
...
describe("UX-C · single source for each fact", () => {
  it("[19][20][21] pick, confidence and odds appear once in the default view", () => {
    ...
    expect(layer("why")!.textContent).not.toMatch(/1\.80|72/);
  });
