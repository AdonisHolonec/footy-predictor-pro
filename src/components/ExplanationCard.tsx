...
export default function ExplanationCard({
  ...
  if (compact) {
    ...
    return (
      <div className="mt-2 space-y-1 border-t border-white/[0.06] pt-2">
        <div className="font-mono text-[8px] uppercase tracking-[0.16em] text-fp-accent/75">{t("panels.reasoning")}</div>
  ...
  const body = (
    <>
      <div className={`mb-4 flex flex-wrap items-end justify-between gap-3 ${framed ? "border-b border-white/5 pb-3" : ""}`}>
        {framed ? (
          <div>
            <h3 className="font-mono text-[10px] uppercase tracking-[0.2em] text-fp-accent/80">
      ...
      <div className="mb-3 font-mono text-[9px] uppercase tracking-[0.16em] text-fp-accent/80">{t("panels.reasoning")}</div>
  ...
  return <section className="rounded-2xl border border-white/5 bg-fp-bg/30 p-5">{body}</section>;
}
