-- 054: say in SQL what the product already says everywhere else — a System is
-- sold in exactly one size.
--
-- The contract is: a System is five selections of which k must win, k in
-- {3, 4, 5}. Migration 053 enforced the k and left the variant to the general
-- allow-list, which accepts 3 and 8 as well. That was harmless while the RPC
-- had exactly one caller and that caller only ever sends 5 — but the next
-- increment puts an HTTP route in front of it, and then the variant stops being
-- a constant the server chose and becomes a value someone sent. A 3-of-3
-- "system" would be storable, gradeable, and meaningless as a product.
--
-- So the restriction lands here, before the API, rather than after it.
--
-- 052 AND 053 ARE NOT EDITED. Both are already applied in production, and
-- Supabase tracks migrations by name: editing either would leave the deployed
-- function untouched while the repository claimed otherwise, and the
-- integration suites would apply the edited copy and go green against a
-- database that never moved. The function is dropped and recreated here,
-- exactly as 050, 052 and 053 did.
--
-- The body below is 053's, generated from it rather than retyped, with ONE
-- change: the invalid_system_variant branch. Nothing else moves — same
-- signature, same defaults, same return type, same SECURITY DEFINER and
-- search_path, same k validation, same combo path, same canonicalisation, same
-- ON CONFLICT identity, same inserts, same ACL.
--
-- The table CHECK from 052 stays as it is. It accepts any k from 2 to variant
-- and it is the second lock, not the first; tightening a constraint on a live
-- table is a heavier operation than reissuing a function, and every write goes
...
begin
  ...
  -- A System is exactly five selections of which k must win. The general
  -- allow-list above answers "is this a variant we sell"; this answers "is this
  -- a variant this KIND is sold in", which for a system is only 5. Ordered
  -- after it on purpose: a variant nobody sells is invalid_variant whatever the
  -- kind, and only a legal variant can be the wrong one for a system.
