...
export async function assertAdmin(req) {
  ...
  if (error) {
    return { ok: false, status: 500, error: error.message || "Unable to load requester profile." };
  }
  if (!profile || profile.role !== "admin") {
    return { ok: false, status: 403, error: "Admin access required." };
  }
  if (profile.is_blocked) {
    return { ok: false, status: 403, error: "Blocked administrators cannot perform this action." };
  }
